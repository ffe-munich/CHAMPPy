...
# Remove clusters 2 and 3
vehicles_df = raw_profiles.vehicles.df
vehicles_df.loc[:, "id_cluster"] = 1
raw_profiles.vehicles.update_vehicles(input_df=vehicles_df)
...
locations_df["label"] = ["Driving", "Depot", "Other location"]
...
user_params_param = champpy.UserParamsParameterizer(
    description="Vans based on REM2030 Dataset (https://www.isi-lehre.de/rem2030-de/REM-2030-Driving-Profiles.php)",  # Define a description for the parameter set
...
user_params_plot = champpy.UserParamsMobPlotter(filename="validation_rem_2030.html", clustering=True)
