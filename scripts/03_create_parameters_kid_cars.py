...
base_dir = os.getcwd()
folder_name = "kid_passenger_cars"
...
locations_df["label"] = ["Driving", "Company site", "Other location"]
...
user_params_param = champpy.UserParamsParameterizer(
    description="Passenger cars based on KID 2010",  # Define a description for the parameter set
    vehicle_type="Passenger car",  # Type of vehicle the parameters apply to (e.g., "Car", "Van", "Truck")
    temp_res=0.25,  # Temporal resolution in hours
)
...
user_params_plot = champpy.UserParamsMobPlotter(filename="validation_kid_cars.html", clustering=True)
