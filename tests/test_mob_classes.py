...
def test_delete_and_add_journeys(mob_data1):
    id_to_delete = [2, 3]
    logbook_df_backup = mob_data1.logbooks.df[mob_data1.logbooks.df["id_journey"].isin(id_to_delete)].copy()
    # Delete journeys
    ...
    for _, row in logbook_df_backup.iterrows():
        assert (
            (mob_data1.logbooks.df["id_vehicle"] == row["id_vehicle"])
            & (mob_data1.logbooks.df["dep_dt"] == row["dep_dt"])
            & (mob_data1.logbooks.df["arr_dt"] == row["arr_dt"])
        ).any()
...
def test_update_journeys(mob_data2):
    mob_data = mob_data2
    update_df = mob_data.logbooks.df[mob_data.logbooks.df["id_journey"].isin([1, 4])].copy()
    update_df.loc[:, "distance"] = 20.0
    mob_data.logbooks.update_journeys(input_df=update_df)
    assert (mob_data.logbooks.df.set_index("id_journey").loc[[1, 4], "distance"] == 20.0).all()
