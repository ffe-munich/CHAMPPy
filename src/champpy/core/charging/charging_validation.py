...
class UserParamsChargingPlotter:
    ...
    clustering: Optional[bool] = False # Option to control whether plots are created for clusters (if defined in the data)
...
class ChargingPlotter:
    ...
    def plot_charging_profiles(self, charge_data: ChargingData):
        ...
        if output_path.suffix != ".html" and self._save_plot:
            logger.warning(f"Can only export as html. Fileformat of output file '{self._filename}'is changed to .html extension.")
    ...
    def plot_charging_char(self, charge_data: ChargingData) -> go.Figure:
        ...
        metrics = ['daily_charging_hours', 'daily_connected_hours', "simultaneous_factor",
                   'daily_driving_consumption', 'daily_charging_energy', 'daily_missing_energy']
        metrics_labels = ['Daily charging hours in h',  'Daily connected hours h', "Simultaneity charging factor",
                           'Daily driving consumption in kWh', 'Daily charging energy in kWh', 'Daily missing energy in kWh']
        ...
        for idx_cluster, cluster in enumerate(clusters):
            ...
            cluster_data = charge_char_df[charge_char_df['id_cluster'] == cluster] if self._clustering else charge_char_df
            ...
            for idx_metrix, metric in enumerate(metrics):
                ...
                show_legend = True if idx_metrix == 0 and n_clusters > 1 else False  # Show legend only for the first metric
    ...
    def plot_load_week(self, charge_data: ChargingData) -> go.Figure:
        ...
        for idx_cluster, cluster in enumerate(unique_id_cluster):
            ...
            def _series(df):
                return df[df['id_cluster'] == cluster].set_index('week_index').reindex(index_week)['power_charging_kw'].tolist()
            ...
            fig.add_trace(go.Scatter(
                ...
                y=y_mean,
                mode='lines',
                line=dict(color=f"rgb({self._rgb_color[6][0] * 255},{self._rgb_color[6][1] * 255},{self._rgb_color[6][2] * 255})", width=2),
                name="Mean",
                showlegend=(idx_cluster == 0),
            ), row=row_idx, col=1)
...
class ChargingCharacteristics():
    ...
    @staticmethod
    def _calc_charge_char(charge_data: ChargingData, 
                            typedays: TypeDays, 
                            grouping: Literal['none', 'vehicle', 'day'] , 
                            method: Literal['mean', 'max', 'min'] , 
                            clustering: Optional[bool] = False) -> pd.DataFrame:
        # Prepare data once
        ...
        for id_cluster in unique_id_cluster:
            for index_typeday in typedays.index:
                ...
                daily_stats.columns = ['energy_consumption', 'power_sum', 'power_count', 'connected_sum', 'energy_missing']
                ...
                simultaneous_charging = charge_df_filtered.groupby('datetime')['power_charging_kw'].apply(lambda x: (x > 0).sum())
                ...
                if grouping == 'none':
                    ...
                    result = {
                        'daily_driving_consumption': daily_driving_consumption.groupby(level='id_vehicle').agg(pd_method).tolist(),
                        'daily_charging_hours': daily_charging_hours.groupby(level='id_vehicle').agg(pd_method).tolist(),
                        'daily_charging_energy': daily_charging_energy.groupby(level='id_vehicle').agg(pd_method).tolist(),
                        'daily_connected_hours': daily_connected_hours.groupby(level='id_vehicle').agg(pd_method).tolist(),
                        'daily_missing_energy': daily_missing_energy.groupby(level='id_vehicle').agg(pd_method).tolist(),
                        'simultaneous_factor': [simultaneous_factor.mean()]
                    }
