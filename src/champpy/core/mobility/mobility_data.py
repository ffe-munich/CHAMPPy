...
class MobData:
	...
	def add_mob_data(self, 
				  input_mob_data: "MobData", 
				  old_cluster_label: str = "Old", 
				  new_cluster_label: str = "New") -> None:
		"""
	...
	def reindexing(self, type: Literal["all", "id_journey", "id_vehicle", "id_cluster"] = "all") -> None:
		...
		if type in ["all", "id_cluster"]:
			...
			cluster_map = {old_cluster: new_cluster for new_cluster, old_cluster in enumerate(sorted(unique_clusters), start=1)}
...
class MobArray():
	...
	def __init__(self, mob_data: MobData):
		...
		(dt_array, _ ) = get_datetime_array(start_date=first_day,
									 end_date=last_day,
									 temp_res=temp_res)
		# Get index in dt_array for start_dt and end_dt
		start_idx = pd.Series(np.searchsorted(dt_array, mob_data_ext_df['start_dt'].values), index=mob_data_ext_df.index)
		...
		all_id_vehicles = np.concatenate([np.full(e - s, vid) for vid, s, e in zip(mob_data_ext_df['id_vehicle'], start_idx, end_idx)])
		all_locations = np.concatenate([np.full(e - s, loc) for loc, s, e in zip(mob_data_ext_df['location'], start_idx, end_idx)])
		all_speeds = np.concatenate([np.full(e - s , spd) for spd, s, e in zip(mob_data_ext_df['speed'], start_idx, end_idx)])
		all_distances = np.concatenate([np.full(e - s, spd) for spd, s, e in zip(mob_data_ext_df['speed'], start_idx, end_idx)])
		all_distances_distributed = np.concatenate([np.full(e - s, dist / (e - s) if e > s else 0) 
								  for dist, s, e in zip(mob_data_ext_df['distance'], start_idx, end_idx)
								  ])
		all_speeds_distributed = np.concatenate([np.full(e - s, spd) for spd, s, e in zip(mob_data_ext_df['speed'], start_idx, end_idx)])
