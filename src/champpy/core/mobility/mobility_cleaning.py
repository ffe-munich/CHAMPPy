...
class UserParamsCleaning:
    ...
    speed: LimitConfig = LimitConfig(
        min_value=0.01, min_method="delete", max_value=120.0, max_method="cap"
    )
    ...
    duration: LimitConfig = LimitConfig(
        min_value=0.25, min_method="delete", max_value=8.0, max_method="cap"
    )
    ...
    distance: LimitConfig = LimitConfig(
        min_value=0.5, min_method="delete", max_value=500.0, max_method="cap"
    )
...
class MobProfilesCleaner:
    ...
    def _clean_column(
        self, mob_profiles: MobProfiles, column: str, update_column: str = None
    ) -> MobProfiles:
        """
        ...
        if mask_above.any():
            if limit_config.max_method == "cap":
                ...
                if column == "speed":
                    ...
                    update_df["arr_dt"] = update_df["dep_dt"] + pd.to_timedelta(
                        limit_config.max_value, unit="h"
                    )
                else:
                    update_df[update_column] = limit_config.max_value

                mob_profiles.logbooks.update_journeys(update_df)
                self.modified_id_journeys[column].extend(
                    lb_df.loc[mask_above, "id_journey"].tolist()
                )
    ...
    def _get_first_last_locations_of_day(
        self, mob_profiles: MobProfiles
        ...
        first_of_day = (
            df.sort_values("dep_dt")
            .groupby(["id_vehicle", "day"])
            .first()
            .reset_index()
        )
        last_of_day = (
            df.sort_values("dep_dt").groupby(["id_vehicle", "day"]).last().reset_index()
        )

        # Count first trips by weekday and location
        first_counts = (
            first_of_day.groupby(["weekday", "dep_loc"])
            .size()
        ...
        last_counts = (
            last_of_day.groupby(["weekday", "arr_loc"])
            .size()
            .reset_index(name="count_last")
        )
    ...
    def _clean_first_last_locations(self, mob_profiles: MobProfiles) -> MobProfiles:
        ...
        merged_counts, id_journeys_first, id_journeys_last = (
            self._get_first_last_locations_of_day(mob_profiles)
        )

        # Get dataframes for first and last of day journeys
        lb_df_only_last = mob_profiles.logbooks.df.loc[
            mob_profiles.logbooks.df["id_journey"].isin(id_journeys_last)
        ]
        lb_df_only_first = mob_profiles.logbooks.df.loc[
            mob_profiles.logbooks.df["id_journey"].isin(id_journeys_first)
        ]
        ...
        most_frequent_first = merged_counts.loc[
            merged_counts.groupby("weekday")["count_first"].idxmax()
        ]
        most_frequent_last = merged_counts.loc[
            merged_counts.groupby("weekday")["count_last"].idxmax()
        ]
        ...
        if not only_first.empty:
            ...
            first_of_day_to_update = first_of_day_to_update.drop(
                columns=["dep_loc"]
            ).merge(
            ...
            vehicle_updates = mob_profiles.vehicles.df[
                mob_profiles.vehicles.df["id_vehicle"].isin(
                    new_first_locs["id_vehicle"]
                )
            ].copy()
            vehicle_updates = (
            ...
            self.modified_id_journeys["location"].extend(
                first_of_day_to_update.index.tolist()
            )
        ...
        if not only_last.empty:
            ...
            last_of_day_to_update = last_of_day_to_update.drop(
                columns=["arr_loc"]
            ).merge(
            ...
            # log modified id_journeys
            self.modified_id_journeys["location"].extend(
                last_of_day_to_update.index.tolist()
            )
    ...
    def _clean_transition_between_days(self, mob_profiles: MobProfiles) -> MobProfiles:
        ...
        raise NotImplementedError(
            "Method _allign_first_last_locations is not implemented yet."
        )
    ...
    def _log_summary(self) -> None:
        ...
        logger.info(
            f"Data has been converted to temporal resolution of {self.params.temp_res:.2f} hours."
        )
        ...
        logger.info(
            "Check deleted_id_journeys and modified_id_journeys attribute for full list."
        )
    ...
    def _log_summary_method(self, method: str) -> None:
        ...
        for key, val in id_journeys.items():
            ...
            summary_lines.append(
                f"  - Due to {key} issues: {len(val)} journeys: id_journeys = {ids_str}"
            )
