...
class MobDataCleaner:
    ...
    def __init__(self, user_params: UserParamsCleaning = None):
        ...
        self.modified_id_journeys = {"distance": [], "speed": [], "duration": [], "location": []}
    ...
    def _get_first_last_locations_of_day(self, mob_data: MobData) -> Tuple[pd.DataFrame, list, list]:
        ...
        merged_counts = pd.merge(
            first_counts, last_counts, left_on=["weekday", "dep_loc"], right_on=["weekday", "arr_loc"], how="outer"
        ).fillna(0)
    ...
    def _clean_first_last_locations(self, mob_data: MobData) -> MobData:
        ...
        if not only_first.empty:
            ...
            first_of_day_to_update = first_of_day_to_update.drop(columns=["dep_loc"]).merge(
                most_frequent_first[["weekday", "dep_loc"]], left_on="weekday", right_on="weekday", how="left"
            )
            ...
            vehicle_updates = mob_data.vehicles.df[
                mob_data.vehicles.df["id_vehicle"].isin(new_first_locs["id_vehicle"])
            ].copy()
            vehicle_updates = (
        ...
        if not only_last.empty:
            ...
            last_of_day_to_update = last_of_day_to_update.drop(columns=["arr_loc"]).merge(
                most_frequent_last[["weekday", "arr_loc"]], left_on="weekday", right_on="weekday", how="left"
            )
